       ...
       real :: LSCTA, LSLCTA, LSLNCTA,LSNTA, LMCTA, LMNTA, BMCTA, BMNTA
       real :: HSCTA, HSNTA, HPCTA, HPNTA
       real :: LSCTP, LSLCTP, LSLNCTP, LSNTP, LMR, LMCTP, LMNTP, BMCTP
       real :: HSCTP, HSNTP, HPCTP, HPNTP
       real :: NCHP, Nf, NCBM, NCHS, ALSLCO2, ALSLNCO2,ALMCO2,ABCO2 
       real :: A1CO2, APCO2, ASCO2, ABP, ASP, A1, ASX, APX
       real :: PRMT_51, PRMT_45
       real :: DF1, DF2, SNMN,  DF3, DF4, DF5, DF6, ADD, ADF1, ADF2 
       real :: ADF3, ADF4, ADF5
       real :: TOT
     ...
     &               * sol_bd(k,j)* 1000. *(1- sol_rock(k,j) / 100.)
          ...
          if (sol_tmp(k,j) > 0. .AND. sol_st(k,j) > 0.) then
              ...
              X1 = 1.0
                  ...
                  !calculate tillage factor using DSSAT
                  if (tillage_switch(j) .eq. 1 .and. tillage_days(j) 
     &              .le. 30) then
                     if (k == 1) then
                     X1 = 1.6
                     else
                         ...
                         elseif (sol_z(k-1,j) .lt. tillage_depth(j))then
                            X1 = 1.0 + 0.6*(tillage_depth(j) - sol_z(
     &                      k-1,j))/(sol_z(k,j) - sol_z(k-1,j))
                         end if                     
                     end if
                  else
                     X1 = 1.0
              ...
              !!When sol_tep is larger than 35, cdg is negative?
                  ...
                  !if (sol_tmp(kk,j) <= 35.0) then
                       ...
                       cdg = sol_tmp(k,j) / (sol_tmp(k,j) + exp(5.058459
     &                  - 0.2503591 * sol_tmp(k,j)))
     ...
     &                      sol_z(kk,j)+sol_z(kk-1,j))/2)))                    
            ...
            !     COMPUTE N/C RATIOS
                  ...
                  X1 = 0.1*(sol_LSN(k,j)+sol_LMN(k,j))/(sol_rsd(k,j)/
     &               1000+1.E-5) !relative notrogen content in residue (%)
                  IF(X1>2.)THEN
                      NCBM=.1
                      GO TO 6
                  END IF
   ...
   21         DF1=LSNTA
              ...
              hmp = 0.
              hmp_rate = 0.
              hmp_rate = 1.4* (HSNTA + HPNTA)/(sol_HSN(k,j) + 
     &           sol_HPN(k,j) + 1.e-6)
              ...
              !! fresh organic n and p (upper two layers only)  
                rmp = 0.             
                decr = 0.
                decr = (LSCTA + LMCTA)/(sol_LSC(k,j) + sol_LMC(k,j) + 
     &             1.e-6)
                decr = min(1., decr)
              ...
              sol_RSPC(k,j)=.3*LSLCTA+A1CO2*(LSLNCTA+LMCTA)+ABCO2*BMCTA+
     &            ASCO2*HSCTA+APCO2*HPCTA
              ...
              sol_cbn(k,j) = 100*(sol_LSC(k,j)+sol_LMC(k,j) +
     &            sol_HSC(k,j)
     &            + sol_HPC(k,j) + sol_BMC(k,j))/sol_mass    
