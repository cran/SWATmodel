...
!! summary calculations
      if (curyr > nyskip) then
        sbactrop = sbactrop + (xx1 - bactrop) * hru_dafr(j)
        sbactrolp = sbactrolp + (xx2 - bactrolp) * hru_dafr(j)
        sbactsedp = sbactsedp + (xx3 - bactsedp) * hru_dafr(j)
        sbactsedlp = sbactsedlp + (xx4 - bactsedlp) * hru_dafr(j)
      end if
